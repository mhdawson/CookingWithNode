...
Server.startServer = function(server) {
  ...
  probes['test'] = {type: 'temp', id: 'test', temp: 25.5, timestamp: (new Date().getTime()/1000), meat: 'beef', taste: 'medium', target: 71};
  probes['test2'] = {type: 'temp', id: 'test2', temp: 25.51, timestamp: new Date().getTime()/1000, meat: 'beef', taste: 'medium', target: 71};
  ...
  eventSocket.on('connection', function(ioclient) {
    ...
    ioclient.on('new-meat', function(deviceId, meat) {
      ...
      if (newTarget === undefined) {
        newTarget = targetTemps[meat][Object.keys(targetTemps[meat])[0]];
        probes[deviceId].target = Object.keys(targetTemps[meat])[0];
      }
