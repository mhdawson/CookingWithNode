...
Server.startServer = function(server) {
  ...
  mqttClient.on('message', function(topic, message) {
    ...
    if (deviceEntry === undefined) {
      deviceEntry = {type: 'temp', id: deviceId, temp: convertTemp(scaleC, temp), timestamp: timestamp,
    ...
    ioclient.on('scaleC', function(deviceId, scaleC) {
      if (probes[deviceId].scaleC != scaleC) {
        ...
        if (scaleC) {
          ...
          probes[deviceId].temp = toC(probes[deviceId].temp)
        } else {
          probes[deviceId].target = toF(probes[deviceId].target);
          probes[deviceId].temp = toF(probes[deviceId].temp)
        }
        eventSocket.emit('data', probes[deviceId]);
      }
    });
